...
function buildUserMessage(epicBody, meta) {
  ...
  // Optionally inject codebase context from referenced files
  if (meta.context_files) {
    const files = meta.context_files.split(',').map(f => f.trim());
    const snippets = [];
    for (const f of files) {
      const abs = path.resolve(process.cwd(), f);
      if (fs.existsSync(abs)) {
        const content = fs.readFileSync(abs, 'utf-8').slice(0, 3000);
        snippets.push(`### ${f}\n\`\`\`\n${content}\n\`\`\``);
      }
    }
    if (snippets.length) {
